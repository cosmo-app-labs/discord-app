...
const selectMenu: SelectMenu = {
    ...
    async execute(interaction) {
        ...
        const { canonicalTitle, endDate, chapterCount, volumeCount, ageRating, averageRating, popularityRank, ratingRank, startDate, status, synopsis, posterImage, titles } = manga.attributes;
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: "💽 No. of Chapters", value: `${chapterCount || "??"}`, inline: true },
                { name: "📚 No. of Volumes", value: `${volumeCount || "??"}`, inline: true },
                {
                    name: "🌟 Average Rating",
        ...
        const selectMenuComponent = createSelectMenu(interaction.message.components[0].components[0].data, selectedMangaId);
