...
const command: Command = {
    ...
    async execute(interaction) {
        ...
        const { name, id, sprites, base_experience, weight, height, types, stats, abilities } = pokemonData;

        const capitalize = (str: string) => str.charAt(0).toUpperCase() + str.slice(1);

        const typeNames = types.map((type: { type: { name: string } }) => capitalize(type.type.name));
        const typeList = typeNames.join(', ');
        const typeLabel = typeNames.length > 1 ? 'Types' : 'Type';

        const abilityNames = abilities.map((ability: { ability: { name: string } }) => capitalize(ability.ability.name));
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                { name: abilityLabel, value: `${abilityList}\n——————————\n${bold('Stats')}` }
            );

        // Add the stats to the embed
        stats.forEach((stat: { stat: { name: string }, base_stat: number }) =>
            embed.addFields({ name: capitalize(stat.stat.name), value: `${stat.base_stat}`, inline: true })
        );
