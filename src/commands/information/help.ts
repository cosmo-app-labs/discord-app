import { ActionRowBuilder, EmbedBuilder, hyperlink, inlineCode, SlashCommandBuilder, StringSelectMenuBuilder } from "discord.js";
...
export function createHelpEmbed(client: Client) {
    return new EmbedBuilder()
        .setColor(config.colors.embed)
        .setAuthor({ name: `${client.user?.username}`, iconURL: client.user?.displayAvatarURL() })
        .setDescription(`Welcome to ${client.user}'s help menu. Here you will find all available commands.`)
        .addFields(
            {
                name: "Commands",
...
export function createSelectMenu(category: Category | "Main Menu") {
    ...
    const categories: { name: Category | "Main Menu", description: string }[] = [
        ...
        { name: "Fun", description: "Commands for games and fun activities" },
        { name: "Images", description: "Commands for generating and fetching images" },
        { name: "Information", description: "Commands to retrieve useful information" },
        { name: "Utility", description: "Helpful utility commands for various tasks" },
    ];
