...
const command: Command = {
    data: new SlashCommandBuilder()
        .setName('trivia')
        .setDescription(
            '💭 Challenge your brain with a fun trivia game and put your knowledge to the test!'
        ),
    category: 'Fun',
    async execute(interaction) {
        ...
        answerCollector.on('collect', async (i) => {
            ...
            const updatedButtons = await createTriviaButtons(
                answers,
                selectedAnswer,
                correct_answer
            );
            ...
            if (correct_answer === selectedAnswer) {
                ...
                return await i.update({
                    content: `❌ Incorrect! The correct answer is ${inlineCode(he.decode(correct_answer))}`,
                    components: [row],
                });
            }
        });
...
export const createTriviaButtons = async (
    answers: string[],
